...
```
GET: 通常用来获取资源
HEAD: 获取资源的元信息（应用，下载大文件时先获取文件大小然后再决定是否下载）
POST: 新增数据
PUT: 修改数据
DELETE: 删除资源
CONNECT: 建立连接隧道，用于代理服务器
OPTIONS: 用于获取目的资源所支持的通信选项（method、header），用来跨域请求
...
其次是对于整数和字符串进行哈夫曼编码，哈夫曼编码的原理一张静态表。
##  多路复用
HTTP2通过二进制分帧，原来Headers + Body的报文格式如今被拆分成了一个个二进制的帧，Headers帧存放头部字段，Data帧存放请求体数据，这些二进制帧不存在先后关系。<br>
