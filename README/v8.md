...
#   对象
##  对象两种访问模式
Dictionary mode（字典模式、哈希表模式） 和 Fast mode（快速模式、只有属性没有方法的 class）<br>
当动态地添加太多属性、删除属性、使用不合法标识符命名属性，那么对象就会变为字典模式。
##  对象的三个指针
...
##  Inline Cache（内联缓存）
Shapes 背后的主要动机是 Inline Caches 或 ICs 的概念。<br>
读属性第一次去查找 shape 链，找到相应属性的 offset，然后记录下来，下次直接使用。 
